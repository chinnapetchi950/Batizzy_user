...
const DeclineScreen = ({searchTerm, declineLengthChange}) => {
  ...
  const GetUserData = useCallback(async () => {
    ...
    await fetch(baseURL + 'renovation_post_requests?status=rejected', {
      method: 'GET',
      headers: {
      ...
      .then(res => {
        ...
        if (res.status === true) {
          ...
          declineLengthChange(res.data.length);
        }
      })
      .catch(error => {
        console.error(error);
  ...
  const Item = ({
    ...
    <Pressable key={'decline' + index} style={styles.item} onPress={onPress}>
  ...
  const renderItem = ({item, index}) => {
    return (
      <Item
        ...
        onPress={() => {
          navigate(routes.DeclineDetailScreen, {
            ItemId: item.id,
          });
        }}
      />
