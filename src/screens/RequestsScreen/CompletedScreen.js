...
const CompletedScreen = ({searchTerm, completedLengthChange}) => {
  ...
  const Item = ({
    ...
    <Pressable key={'completed' + index} style={styles.item} onPress={onPress}>
  ...
  const GetUserData = useCallback(async () => {
    ...
    await fetch(baseURL + 'renovation_post_requests?status=completed', {
      method: 'GET',
      headers: {
      ...
      .then(res => {
        ...
        if (res.status === true) {
          ...
          completedLengthChange(res.data.length);
        }
      })
      .catch(error => {
        console.error(error);
      });
  }, [completedLengthChange]);
  ...
  const renderItem = ({item, index}) => {
    return (
      <Item
        ...
        onPress={() => {
          navigate(routes.CompletedRequestScreen, {
            ItemId: item.id,
          });
        }}
      />
