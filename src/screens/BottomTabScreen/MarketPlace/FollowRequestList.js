...
const FollowRequestList = ({searchTerm}) => {
  ...
  const getPendingRequest = useCallback(async () => {
    ...
    const response = await fetch(`${baseURL}pending-follow-requests`, {
      method: 'GET',
      headers: {
  ...
  const ConfrimFollowRequest = async userID => {
    ...
    await fetch(baseURL + 'follow-request/' + userID + '/' + 'accept', {
      method: 'POST',
      headers: {
        Authorization: 'Bearer ' + Token,
        Accept: 'application/json',
  ...
  const DeleteFollowRequest = async userID => {
    setIsLoading(true);
    var Token = await AsyncStorage.getItem('accessToken');
    await fetch(baseURL + 'follow-request/' + userID + '/' + 'reject', {
      method: 'POST',
      headers: {
        Authorization: 'Bearer ' + Token,
        Accept: 'application/json',
